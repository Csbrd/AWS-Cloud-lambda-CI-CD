...
BigQuery lifesync_curated.ai_feature_table 에서 29개 피처를 읽어
...
def train_signup(df: pd.DataFrame) -> str:
    print("\n[2/4] 가입 예측 모델 (XGBClassifier, proxy=inactive_days>30)")
    X = df[["row_id"] + NUMERIC_FEATURES].fillna(0.0)
...
def train_rec(df: pd.DataFrame) -> str:
    print("\n[3/4] 추천 예측 모델 (XGBClassifier, proxy=invest_ratio>0.3)")
    X = df[["row_id"] + NUMERIC_FEATURES].fillna(0.0)
