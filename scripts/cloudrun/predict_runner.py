...
def _ensure_feature_table():
    ...
    - ai_feature_pred      : row_id + 29 피처 = 30 컬럼 (VIP / Signup / Rec 모델용)
    - ai_feature_pred_health: row_id + 28 피처 = 29 컬럼, health_risk_score 제외 (Health 모델용)
...
_SERVING_SQL = """
    ...
    scored AS (
        SELECT
            ...
            -- DynamicScore = (BaseScore×0.70) + (VIP×15) + (Signup×10) + (Rec×10)
            --              + (Health×0.10) + (Behavior×0.10) - (Churn×0.05)
            ROUND(
                LEAST(100.0, GREATEST(0.0,
                    COALESCE(f.lifesync_score, 0.0)  * 0.70
