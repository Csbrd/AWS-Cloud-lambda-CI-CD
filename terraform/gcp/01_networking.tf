...
# PGA (Private Google Access for Hybrid) — 199.36.153.0/24 restricted 대역
# AWS↔GCP VPN 경유 시 *.googleapis.com 을 이 IP 로 해석해 Google 백본 경유
resource "google_dns_record_set" "pga_googleapis_a" {
  name         = "googleapis.com."
  type         = "A"
  ttl          = 300
  managed_zone = google_dns_managed_zone.googleapis.name
...
# ── Cloud Router — VPN + PGA 대역 BGP 광고 ───────────────────────────────────
# 199.36.153.0/24 광고 → AWS TGW propagated 라우팅 테이블에서 확인 가능
resource "google_compute_router" "vpn_router" {
  name    = "lifesync-vpn-router"
  network = google_compute_network.vpc.id
  region  = var.region
  ...
  bgp {
    ...
    advertised_ip_ranges {
      range       = "199.36.153.0/24"
      description = "PGA restricted range — AWS admin to GCP API"
    }
  }
}
